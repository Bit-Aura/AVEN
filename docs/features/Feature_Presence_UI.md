...
```mermaid
flowchart TD
    A[User Connects] --> B[Establish WebSocket]
    B --> C[Broadcast Active Status]
    C --> D[Update Peer Avatars]
```
Or in text form:
1. User connects to the platform.
2. A WebSocket heartbeat is established.
3. The user's active status is broadcast to their cohort.
4. Peer avatars light up in real-time across the UI.
