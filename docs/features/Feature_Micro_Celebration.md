...
```mermaid
flowchart TD
    A[Node Mastered] --> B[Trigger Celebration Event]
    B --> C[Render Confetti Animation]
    C --> D[Play Success SFX]
    D --> E[Dismiss Overlay]
```
Or in text form:
1. The learner proves mastery on a node.
2. A celebration event is emitted globally.
3. Confetti animations and UI micro-interactions render instantly.
4. The overlay gracefully dismisses after 2 seconds.
