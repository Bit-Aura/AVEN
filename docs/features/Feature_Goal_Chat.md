...
```mermaid
flowchart TD
    A[Learner Starts Goal Chat] --> B[AI Evaluates Current Goal]
    B --> C[Fetch Relevant Graph Nodes]
    C --> D[AI Recommends Adjustments]
    D --> E[Learner Confirms]
    E --> F[Update Path Store]
```
Or in text form:
1. Learner opens the Goal Chat.
2. AI assesses the current active goal against graph progression.
3. System fetches optimal path adjustments from Neo4j.
4. Path is dynamically rerouted based on learner confirmation.
