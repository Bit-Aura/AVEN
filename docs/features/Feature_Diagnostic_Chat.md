...
- **Clean Environment Compliance**: Maintained our stringent testing configurations to guarantee absolutely zero benchmark or provisional files are persisted, keeping the repository completely flag-free (Rule 7).
...
```mermaid
flowchart TD
    A[New Learner Signup] --> B[Trigger Diagnostic Chat]
    B --> C[AI Asks Evaluation Questions]
    C --> D[Learner Responds]
    D --> E{Confidence High?}
    E -- No --> C
    E -- Yes --> F[Generate Initial BKT Graph]
```
Or in text form:
1. A new learner signs up.
2. A diagnostic chat session is initiated.
3. The AI evaluates the learner's baseline knowledge.
4. An initial Bayesian Knowledge Tracing (BKT) graph is generated based on responses.
