...
### Feature 13: Job Data Collection & Scraping Pipeline (`app/scraper/`)
- Asynchronous, source-agnostic ETL subsystem for extracting, cleaning, validating, and deduplicating live market job postings (e.g. Greenhouse boards).
- Features: Zero-dependency HTML-to-plain-text normalizer, automatic job-type heuristic classification (internship, contract, full_time), location/date standardization, in-memory duplicate resolution, and a standalone CLI tool (`app/scraper/cli.py`).
- Full documentation available in `docs/Feature_Job_Scraping_Pipeline.md`.
...
| `/api/v1/scraper/sources` | GET | None | Returns supported job board source adapters |
| `/api/v1/scraper/scrape` | POST | `ScrapeJobsInput` | Triggers job collection, HTML cleaning, normalization & deduplication |
...
- **Scraper Test Suite**: 436 lines of comprehensive unit/mock tests passing via `apps/api/tests/test_scraper.py`.
