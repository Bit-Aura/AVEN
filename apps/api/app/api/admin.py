...
from app.models.domain import User, MentorApplication, Resource, ResourceMetadata, SkillRecord, RoadmapCache, RoadmapIngestionConflict, RoleRoadmapMapping
from app.infrastructure.neo4j.client import neo4j_client
from app.infrastructure.roadmap_client import roadmap_client
from app.services.roadmap_ingestion import roadmap_ingestion_service
from app.schemas.roadmap_schemas import (
    RoadmapAvailableResponse,
    RoadmapItemSchema,
    RoadmapSyncResponse,
    RoadmapConflictsListResponse,
    RoadmapConflictItemSchema,
    RoleRoadmapMappingsListResponse,
    RoleRoadmapMappingUpdateInput,
    RoadmapPreviewResponse,
    NormalizedSkillPreview,
    CandidateEdgePreview,
)
...
# =============================================================================
# 6. ROADMAP.SH CANONICAL TOPOLOGY MANAGEMENT
# =============================================================================

@router.get("/admin/roadmaps/available", response_model=RoadmapAvailableResponse)
async def get_available_roadmaps(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_admin)
):
    """
    Lists available roadmaps from roadmap.sh catalog along with local PostgreSQL cache status.
    """
    # 1. Fetch cached records
    stmt = select(RoadmapCache)
    cached_rows = (await db.execute(stmt)).scalars().all()
    cached_map = {c.slug: c for c in cached_rows}

    # 2. Fetch catalog list (1 credit or cached fixture)
    catalog = await roadmap_client.list_roadmaps()
    items: List[RoadmapItemSchema] = []

    for item in catalog:
        slug = item["slug"]
        c_row = cached_map.get(slug)
        items.append(
            RoadmapItemSchema(
                slug=slug,
                title=item.get("title", slug.replace("-", " ").title()),
                updatedAt=item.get("updatedAt"),
                cached=c_row is not None,
                fetched_at=c_row.fetched_at if c_row else None,
                credits_spent=c_row.credits_spent if c_row else 0
            )
        )

    return RoadmapAvailableResponse(roadmaps=items, total=len(items))


@router.post("/admin/roadmaps/sync/{slug}", response_model=RoadmapSyncResponse)
async def trigger_roadmap_sync(
    slug: str,
    force: bool = Query(False, description="Force remote fetch bypassing cache"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_admin)
):
    """
    Manually triggers Stage B-F ingestion pipeline for one roadmap slug.
    """
    res = await roadmap_ingestion_service.sync_roadmap_slug(slug, db, force=force)
    return RoadmapSyncResponse(**res)


@router.get("/admin/roadmaps/conflicts", response_model=RoadmapConflictsListResponse)
async def get_roadmap_conflicts(
    resolved: Optional[bool] = Query(None, description="Filter by resolution status"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_admin)
):
    """
    Surfaces roadmap topology conflicts (cycles, edge mismatches, unmapped market skills).
    """
    stmt = select(RoadmapIngestionConflict).order_by(RoadmapIngestionConflict.created_at.desc())
    if resolved is not None:
        stmt = stmt.where(RoadmapIngestionConflict.resolved == resolved)
    ...
    return RoadmapConflictsListResponse(conflicts=items, total=len(items))


@router.patch("/admin/roadmaps/conflicts/{conflict_id}")
async def resolve_roadmap_conflict(
    conflict_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_admin)
):
    """
    Marks an ingestion conflict item as resolved.
    """
    stmt = select(RoadmapIngestionConflict).where(RoadmapIngestionConflict.id == conflict_id)
...
@router.get("/admin/roadmap-role-mapping", response_model=RoleRoadmapMappingsListResponse)
async def get_role_roadmap_mappings(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_admin)
):
    """
    Retrieves current role-to-roadmap composite mappings.
    """
    roles = ["backend_swe", "frontend_swe", "devops_platform", "mlops_engineer", "data_engineer"]
    mappings: Dict[str, List[str]] = {}

    for r in roles:
        slugs = await roadmap_ingestion_service.get_role_roadmap_slugs(r, db)
        mappings[r] = slugs

    return RoleRoadmapMappingsListResponse(mappings=mappings)


@router.patch("/admin/roadmap-role-mapping")
async def update_role_roadmap_mapping(
    payload: RoleRoadmapMappingUpdateInput,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_admin)
):
    """
    Updates the roadmap slugs backing a specific role.
    """
    # Delete existing mapping rows for this role
    await db.execute(delete(RoleRoadmapMapping).where(RoleRoadmapMapping.role_id == payload.role_id))
    await db.flush()

    # Insert new mapping rows
    for idx, slug in enumerate(payload.roadmap_slugs):
        mapping_row = RoleRoadmapMapping(
    ...
    await db.commit()
    return {"status": "success", "role_id": payload.role_id, "roadmap_slugs": payload.roadmap_slugs}


@router.get("/admin/roadmaps/{slug}/preview", response_model=RoadmapPreviewResponse)
async def preview_roadmap_slug(
    slug: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_active_user)
):
    """
    Returns normalized Skill nodes and Candidate edges for the given roadmap slug.
    Open to any authenticated user (learner or admin) for read-only graph rendering.
    """
    cache_row = await roadmap_ingestion_service.fetch_and_cache_roadmap(slug, db, force=False)
    ...
    edge_previews = [
        CandidateEdgePreview(source_id=u, target_id=v, type="PREREQUISITE_OF")
        for u, v in edges
    ]

    return RoadmapPreviewResponse(
        slug=slug,
        skills=skill_previews,
        edges=edge_previews,
        is_dag=len(cycles) == 0,
        cycles=cycles,
        total_skills=len(skill_previews),
        total_edges=len(edge_previews)
    )


@router.get("/learner/roadmaps/{slug}/graph", response_model=RoadmapPreviewResponse)
async def learner_roadmap_graph(
    slug: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_active_user)
):
    """
    Learner-accessible alias: returns the canonical Skill node flowchart and edges
    for a roadmap slug without requiring admin privileges.
    """
    cache_row = await roadmap_ingestion_service.fetch_and_cache_roadmap(slug, db, force=False)
    clean_nodes = cache_row.clean_nodes_json or []
    raw_detail = cache_row.raw_detail_json or {}
