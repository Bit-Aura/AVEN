...
compatibility: Requires NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY and CLERK_SECRET_KEY (or framework-specific equivalents like VITE_CLERK_PUBLISHABLE_KEY for Vite-based apps). Keys can be auto-generated by `clerk init` (provisional development keys, no Clerk account required), or pulled from the Clerk Dashboard. Requires Node.js 20.9.0 or higher.
metadata:
  author: clerk
  version: 2.5.0
---
...
**No login required.** Unauthenticated, `clerk init` writes provisional development keys to the project's env file — no account, no browser, no flag. Don't run `clerk auth login` first. Authenticated (or with `--app` / `--login`) it creates and links a real app via PLAPI instead.

`--template <b2b-saas|b2c-saas|native|waitlist>` pre-configures the provisional app. Caveats — a signed-out human in an *existing* project still gets the login flow unless they pass `--keyless`; `--template`/`--fresh` error on any run that targets a real app; login only auto-claims what `clerk init` created. See [clerk-cli](../clerk-cli/references/auth.md#accountless-operating-without-an-account).
...
| `@remix-run/react` | Remix (phased out) | Migrate to React Router v7 — use the React Router quickstart below |
...
**CLI (Automatic)**
- `clerk init` writes provisional development keys to the project's env file — no Clerk account required
...
2. **Create migration plan** - Consider:
   ...
   - **External IDs** - Store established user IDs as `external_id` in Clerk
