...
export default function PresenceBar() {
  ...
  return (
    <div className="flex items-center gap-3 bg-slate-900/80 border border-slate-700 px-4 py-2 rounded-full shadow-lg backdrop-blur-sm animate-in fade-in zoom-in">
      <div className="flex items-center gap-2 text-aven-text-subtle border-r border-slate-700 pr-3">
        <Users size={16} />
      ...
      <div className="flex -space-x-2 overflow-hidden">
        {onlineCollaborators.map((collaborator) => (
          <div 
            key={collaborator.id} 
            className={`relative inline-block h-8 w-8 rounded-full ring-2 ring-slate-900 ${collaborator.color} flex items-center justify-center text-aven-text text-xs font-bold shadow-md cursor-help group transition-transform hover:scale-110 hover:z-10`}
          ...
          >
            ...
            {/* Tooltip */}
            <div className="absolute top-10 hidden group-hover:block whitespace-nowrap bg-slate-800 text-aven-text text-xs px-2 py-1 rounded border border-slate-600 shadow-xl z-50">
              {collaborator.name}
            </div>
          </div>
        ))}
