  ...
  return (
    <div className="bg-aven-base border border-aven-border rounded-2xl p-5 shadow-glass space-y-3 max-w-sm">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold uppercase tracking-wider text-aven-text-subtle flex items-center gap-1.5">
      ...
      <div className="flex -space-x-2 pt-1">
        {members.map((initials, idx) => (
          <div 
            key={idx} 
            className="w-8 h-8 rounded-full border-2 border-surface bg-brand-600 text-aven-text font-bold text-xs flex items-center justify-center shadow-sm"
          >
            {initials}
          </div>
        ))}
