...
describe('IdeSidecar Component', () => {
  it('renders IDE sidecar', () => {
    (usePathStore as unknown as jest.Mock).mockImplementation((selector) => selector({ ...baseMock, activeIdeNodeId: 'node-1' }));
    render(<IdeSidecar />);
    expect(screen.getByText(/IDE Environment/i)).toBeInTheDocument();
  });
});