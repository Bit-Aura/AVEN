...
describe('TrustPanel Component', () => {
  it('renders readiness stats and explanation', () => {
    (usePathStore as unknown as jest.Mock).mockImplementation((selector) => selector({ ...baseMock, isTrustPanelOpen: true }));
    render(<TrustPanel />);
    expect(screen.getByText(/Readiness Vector/i)).toBeInTheDocument();
  });
});